...
# coding: utf-8
"""
Compare MERFISH-MERFISH full LDDMM alignment results between STalign and Squidpy.
"""
...
stalign_file = DATA_DIR / 'output_stalign/stalign_merfish_merfish_lddmm_results.csv.gz'
...
squidpy_file = DATA_DIR / 'output/squidpy_merfish_merfish_lddmm_results.csv.gz'
...
if has_stalign:
    ...
    axes[plot_idx].scatter(xJ, yJ, s=1, alpha=0.1, label='target')
    axes[plot_idx].set_title('After STalign LDDMM', fontsize=14)
    axes[plot_idx].legend(markerscale=5)
    axes[plot_idx].set_aspect('equal')
    plot_idx += 1
...
if has_squidpy:
    ...
    axes[plot_idx].scatter(xJ, yJ, s=1, alpha=0.1, label='target')
    axes[plot_idx].set_title('After Squidpy LDDMM', fontsize=14)
    axes[plot_idx].legend(markerscale=5)
    axes[plot_idx].set_aspect('equal')

plt.tight_layout()
output_fig = OUTPUT_DIR / 'merfish_merfish_lddmm_comparison.png'
