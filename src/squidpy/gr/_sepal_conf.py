...
# Default numba configuration options
DEFAULT_NUMBA_CONFIG = {
    "fastmath": True,
    "parallel": False,
    "cache": False,
    "error_model": "numpy",  # Options: "python", "numpy"
    "boundscheck": False,
    "nopython": True,
}

# Store different function versions
_NUMBA_FUNCTION_CACHE = {}


def create_numba_function(func, config):
    """Create a numba-decorated version of a function with specific config."""
    # Create a unique key for this function + config
    config_key = tuple(sorted(config.items()))
    cache_key = (func.__name__, config_key)

    # Return cached version if exists
    if cache_key in _NUMBA_FUNCTION_CACHE:
        return _NUMBA_FUNCTION_CACHE[cache_key]

    # Create new decorated function
    config_copy = config.copy()
    # Extract special case for parallel which only applies to specific functions
    use_parallel = config_copy.pop("parallel", False)

    # For laplacian_rect, we may want parallel=False explicitly
    if func.__name__ == "_laplacian_rect":
        decorator = njit(parallel=use_parallel, **config_copy)
    else:
        decorator = njit(**config_copy)

    decorated_func = decorator(func)
    _NUMBA_FUNCTION_CACHE[cache_key] = decorated_func
...
def sepal(
    ...
    numba_config: Dict[str, Any] = None,
    ...
    *Sepal* is a method that simulates a diffusion process to quantify spatial structure in tissue.
    ...
    %(parallelize)s
    numba_config
        Dictionary of numba configuration options. If None, uses default settings.
        Options include:
            - fastmath (bool): Enable fast-math optimizations (default: True)
            - parallel (bool): Enable automatic parallelization (default: False)
            - cache (bool): Enable caching of compiled function (default: False)
            - error_model (str): Error model, 'python' or 'numpy' (default: 'numpy')
            - boundscheck (bool): Enable bounds checking (default: False)
    config_name
        Name for the configuration used (for comparison purposes)
    ...
    The numba compilation options can significantly affect both performance and numerical results.
    Set `fastmath=False` for more numerically stable results, though slower execution.
    """
    # Apply default config if none provided
    if numba_config is None:
        numba_config = DEFAULT_NUMBA_CONFIG.copy()
    else:
        # Start with defaults and update with user options
        config = DEFAULT_NUMBA_CONFIG.copy()
        config.update(numba_config)
        numba_config = config

    # Log the configuration being used
    logg.info(f"Using numba config: {config_name} - {numba_config}")

    # Create optimized versions of numerical functions
    _diffusion_func = create_numba_function(_diffusion_impl, numba_config)
    _laplacian_rect_func = create_numba_function(_laplacian_rect_impl, numba_config)
    _laplacian_hex_func = create_numba_function(_laplacian_hex_impl, numba_config)
    _entropy_func = create_numba_function(_entropy_impl, numba_config)
    ...
    score = parallelize(
        ...
        dt=dt,
        thresh=thresh,
        _diffusion_func=_diffusion_func,
        _laplacian_rect_func=_laplacian_rect_func,
        _laplacian_hex_func=_laplacian_hex_func,
        _entropy_func=_entropy_func,
    )

    key_added = f"sepal_score_{config_name}" if config_name != "default" else "sepal_score"
...
def _score_helper(
    ...
    _diffusion_func=None,
    _laplacian_rect_func=None,
    _laplacian_hex_func=None,
    _entropy_func=None,
) -> NDArrayA:
    if max_neighs == 4:
        fun = _laplacian_rect_func
    elif max_neighs == 6:
        fun = _laplacian_hex_func
    else:
    ...
    for i in ixs:
        ...
        time_iter = _diffusion_func(
            conc, fun, n_iter, sat, sat_idx, unsat, unsat_idx, dt=dt, thresh=thresh, _entropy_func=_entropy_func
        )
...
# Implementation functions without decorators
def _diffusion_impl(
    conc: NDArrayA,
...
# Laplacian functions without decorators
def _laplacian_rect_impl(
    centers: NDArrayA,
    nbrs: NDArrayA,
    h: float,
) -> NDArrayA:
