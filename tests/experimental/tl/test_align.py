"""Integration tests for the public ``align_stalign`` function.
...
from squidpy.experimental.tl import align_stalign
...
def test_object_mode_returns_result_and_touches_nothing() -> None:
    ...
    result = align_stalign(ref, query, output_mode="object", **_TINY)
...
def test_inplace_writes_explicit_key() -> None:
    ref, query = _adata(), _adata()
    out = align_stalign(ref, query, output_mode="inplace", key_added="spatial_aligned", **_TINY)
    assert out is None
...
def test_inplace_default_key() -> None:
    ref, query = _adata(), _adata()
    align_stalign(ref, query, output_mode="inplace", **_TINY)
    assert "aligned_spatial" in query.obsm


def test_copy_leaves_original_untouched() -> None:
    ref, query = _adata(), _adata()
    out = align_stalign(ref, query, output_mode="copy", key_added="aligned", **_TINY)
...
def test_image_not_implemented() -> None:
    ...
    with pytest.raises(NotImplementedError, match="on='image'"):
        align_stalign(ref, query, on="image", **_TINY)


def test_missing_spatial_key() -> None:
    ref, query = _adata(), _adata()
    with pytest.raises(KeyError, match="missing.*not found"):
        align_stalign(ref, query, spatial_key="missing", **_TINY)
...
def test_object_result_satisfies_align_result_protocol() -> None:
    ...
    result = align_stalign(_adata(), _adata(), output_mode="object", **_TINY)
...
def test_sdata_object_mode() -> None:
    ...
    result = align_stalign(sdata, ref_key="ref", query_key="query", output_mode="object", **_TINY)
    assert isinstance(result, StalignResult)
...
def test_sdata_copy_leaves_original_untouched() -> None:
    ...
    sdata = _sdata_tables(ref=_adata(), query=_adata())
    out = align_stalign(sdata, ref_key="ref", query_key="query", output_mode="copy", key_added="aligned", **_TINY)
