...
def test_score_perms_matches_scanpy_per_permutation(mode: str):
    ...
    got = _score_perms(g, vals, autocorr, n_perms, 0, 1, False)
...
def test_score_perms_thread_invariant(mode: str):
    ...
    autocorr = SpatialAutocorr(mode)
    serial = _score_perms(g, vals, autocorr, n_perms, 0, 1, False)
    threaded = _score_perms(g, vals, autocorr, n_perms, 0, 4, False)
